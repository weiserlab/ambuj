...
The goal of this project is to develop a framework to manage large-scale deployments of flexible computers (STICORS) that are packed with sensors, processors, energy storage, and human-computer interaction mechanisms, and that can connect wirelessly over long distances. These computers will be able to handle a range of advanced computing and communication tasks. Importantly, we are developing a framework for the deployment and management of large numbers of STICORS. This framework has the potential to support a wide range of applications, but currently, we are focusing on creating a platform for near real-time analytics on massive amounts of data collected from the physical world using STICORS computers.
