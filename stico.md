...
The goal of this project is to design and build flexible computers (STICORS) that are packed with sensors, processors, energy storage, and human-computer interaction mechanisms, and that can connect wirelessly over long distances. These computers will be able to handle a range of advanced computing and communication tasks. In addition, we are developing a framework for the deployment and management of large numbers of STICORS. This framework could enable a number of applications, but at present, our focus is on developing an analytics platform that allows for the massive collection of data from the physical world using STICORS and the ability to perform near real-time analytics on this data.
